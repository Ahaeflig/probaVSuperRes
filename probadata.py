...
class ProbaVDataset():
    ...
    def __init__(self, data_dir: str):
        ...
        self.train_cycle = cycle(self.train_samples)
        self.test_cycle = cycle(self.test_samples)
    ...
    """
    @tf.function
    def get_uplr_hr(self, scene, augment=False):
        lr, hr = self.load_image(scene.uplr), self.load_image(scene.hr)
        
        if augment:
            return self.random_jitter(lr, hr)
        else:
            return lr, hr
    ...
    def random_crop(self, lr, hr):
        stacked_image = tf.stack([lr, hr], axis=0)
        cropped_image = tf.image.random_crop(stacked_image, size=[2, 384, 384, 1])
        return cropped_image[0], cropped_image[1] 
        
    @tf.function()
    def random_jitter(self, lr, hr):
        lr, hr = self.resize(lr, 429, 429), self.resize(hr, 429, 429)
    ...
    def lr_hr_gen(self):
        yield next(self.train_cycle), next(self.test_cycle), 
